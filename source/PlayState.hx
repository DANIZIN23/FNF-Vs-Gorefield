...
class PlayState extends MusicBeatState
{
	...
	// if (SONG.song == 'South')
		...
		DiscordClient.changePresence(detailsText, SONG.song + " (" + "HARD" + ")", iconP2.getCharacter());
		#end
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				{
					case 0:
						FlxG.sound.play(Paths.sound('3' + introSoundsSuffix), 0.6);
					case 1:
						...
						FlxG.sound.play(Paths.sound('2' + introSoundsSuffix), 0.6);
					case 2:
						...
						FlxG.sound.play(Paths.sound('1' + introSoundsSuffix), 0.6);
					case 3:
						if (!skipCountdown){
						...
						FlxG.sound.play(Paths.sound('Jon' + introSoundsSuffix), 0.6);
						}
					case 4:
				}
	...
	function startSong():Void
	{
		...
		DiscordClient.changePresence(detailsText, SONG.song + " (" + "HARD" + ")", iconP2.getCharacter(), true, songLength);
		#end
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if (startTimer.finished)
			{
				DiscordClient.changePresence(detailsText, SONG.song + " (" + "HARD" + ")", iconP2.getCharacter(), true, songLength - Conductor.songPosition - ClientPrefs.noteOffset);
			}
			else
			{
				DiscordClient.changePresence(detailsText, SONG.song + " (" + "HARD" + ")", iconP2.getCharacter());
	...
	override public function onFocusLost():Void
	{
		...
		if (health > 0 && !paused)
		{
			DiscordClient.changePresence(detailsPausedText, SONG.song + " (" + "HARD" + ")", iconP2.getCharacter());
		}
		#end
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				DiscordClient.changePresence("Game Over - " + detailsText, SONG.song + " (" + "HARD" + ")", iconP2.getCharacter());
				#end
				isDead = true;
				return true;
			}
